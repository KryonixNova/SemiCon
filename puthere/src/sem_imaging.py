...
def add_shot_noise(img: np.ndarray, dose: float, rng: np.random.Generator) -> np.ndarray:
    ...
    # `rng.poisson` raises on a non-finite lambda (NaN/inf) *and* on a
    # finite lambda past its own internal ceiling (~2**63) -- a single bad
    # element kills the whole DataLoader worker, not just one sample. Last
    # line of defense against either case reaching here from an upstream
    # numeric edge case -- unreachable today (img is always uint8, and
    # `dose` is already sanitized above, so `counts` can't actually be
    # non-finite here; this only guards a hypothetical future caller that
    # passes an already-corrupted `img`). +inf reads as "saturated bright,"
    # not "no signal": map it to the same 1e12 ceiling counts get clipped
    # to below, not 0.0, so a corrupted-bright pixel stays bright rather
    # than silently going dark. 1e12 is far above any physically plausible
    # photon count (img in [0, 255], dose in the low thousands) yet safely
    # under numpy's limit -- unlike a posinf->float64-max substitution,
    # which is itself past that limit and trades one crash for another.
