...
Example:
    python scripts/ablation_jitter.py --checkpoint checkpoints/m3_hn_r24/best.pt
"""
...
def main():
    ...
    for tr, row in results.items():
        print(f"{tr:<14} " + " ".join(f"{row[tp]['acc@50px']:>12.3f}" for tp in PROFILES))

    if "normal" in results:
        base = results["normal"]["normal"]["acc@50px"]
        for tp in ("zero", "shifted"):
            drop = base - results["normal"][tp]["acc@50px"]
