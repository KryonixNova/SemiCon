"""AFB's DRAM SEM-style synthetic image generator -- extracted from
AFB/dram_localization_pipeline.py for the KryonixNova hackathon submission.

Generates synthetic DRAM SEM reference/search image pairs from procedural
GDS-style layout + rasterization + imaging-noise models:
...
AFB's own SuperPoint+LightGlue matcher and RANSAC localizer (sections 4-6
of the original single-file build) are intentionally excluded from this
submission -- puthere's deep-learned localizer (see ../puthere/) is the
one localization approach used here, so this file has no torch/lightglue
dependency.
...
class DRAMParams:
    ...
    # ── Array geometry ──────────────────────────────────────────────────────
    rows: int = 64
    cols: int = 85   # rows*cell_pitch_nm == cols*cell_pitch_bl_nm (both 5100-5120nm):
                      # array is square, so it fills the 1000x1000 canvas with no margin
    ...
    # generate_sample() sets this False: it writes to a tmp_dir, uses the
    # in-memory meta dict directly, and unlinks output_json without ever
    # reading it back -- the metadata dict there can hold tens of thousands
    # of cell/wordline/bitline coordinates, so json.dumps(..., indent=2)
    # writing it to disk purely to delete it moments later was pure waste.
    ...
    zoom_ratio: int = 1               # reference render resolution multiplier;
                                       # 1 = reference_img is literally 100x100px
                                       # (same pixel density as the search image)
    ...
    die_block_width_nm: float = 5120.0   # per-block physical footprint, matches height so
    die_block_height_nm: float = 5120.0  # each block is square too (no per-block margin in NxM dies)
    die_street_width_nm: float = 100.0    # gap rendered between adjacent blocks
    die_block_variation_frac: float = 0.4   # +/- bound on per-block parameter jitter

    # If set, each die block draws its own base preset from this list (see
    # DRAM_PRESETS) before the usual per-block jitter is applied, instead of
    # jittering around a single fixed base -- lets one die mix DRAM
    # densities across blocks. None (the default) is an exact passthrough:
    # every block jitters around this DRAMParams' own fields, same as today.
    block_presets: Optional[List[str]] = None
    # Probability [0, 1] that the reference-patch crop is deliberately
    # chosen to straddle a block/street seam instead of falling uniformly
    # inside one block. 0.0 (the default) is an exact passthrough. No effect
    # when die_block_rows == die_block_cols == 1 (no seams to straddle).
...
# ── DRAM density presets ────────────────────────────────────────────────────
# Named density variants, illustrative of publicly known DRAM scaling trends --
# NOT exact proprietary fab specifications (same disclaimer as the module
# docstring's process-node note). Each preset scales AFB's own defaults
# ("dram_1x", the identity case) by a single factor across all six fields, so
# the square-array-no-margin invariant (rows*cell_pitch_nm == cols*
# cell_pitch_bl_nm, at the default rows=64/cols=85) holds for every preset
...
def resolve_preset(name: str) -> dict:
    """Return a copy of DRAM_PRESETS[name]'s field overrides. Raises KeyError
    (with the valid name list) if name isn't a known preset."""
...
class ProcessVariation:
    ...
    def add_ler(self, box: db.Box, axis: str,
        ...
        # One batched draw of both per-segment perturbations instead of two
        # separate rng.normal() calls inside the loop -- NumPy's Generator
        # consumes its underlying bitstream in the same row-major sequence
        # either way (verified bit-identical, including post-call state,
        # against the original per-segment-call form), so this is a pure
        # speedup: (2n-1) fewer Python-level rng call overheads per line.
        if axis == 'x':
...
def _canvas_bbox_nm(pixels_per_nm: float, width_px: int, height_px: int,
    ...
    search image can have, including any flat-background margin left when
    the array/die isn't square (see render_sem_image).
...
class DRAMGenerator:
    ...
    def __init__(self, params: DRAMParams) -> None:
        ...
        # Validate rasterization params before any layout work. Search image
        # size is locked (benchmark compatibility); reference size and zoom
        # are free positive parameters -- render_sem_patch already supports
        # arbitrary ppn/width/height, this is the only thing gating it.
...
def square_die_block_width_nm(rows: int, cols: int, street_width_nm: float,
    ...
    """Per-block width (nm) that makes an NxM die's total footprint exactly
    square (total_w_nm == total_h_nm), for the given block_height_nm and
    street_width_nm. Square blocks (die_block_width_nm == die_block_height_nm)
    only yield a square die when rows == cols; for any other grid shape the
    block width must compensate, which is what this solves:

        cols*block_w + (cols-1)*street = rows*block_h + (rows-1)*street

    Reduces to block_height_nm when rows == cols (the (rows-cols) term
    vanishes), matching the default square-block behavior in that case.
    """
...
class DieGenerator:
    ...
    def _boundary_biased_crop_origin(self, ref_br: int, ref_bc: int,
        ...
        """Pick a crop origin (x0, y0) that straddles a block/street seam
        adjacent to block (ref_br, ref_bc), instead of the usual
        uniform-inside-one-block placement. Picks a random interior seam
        (vertical if die_block_cols > 1, horizontal if die_block_rows > 1,
        whichever axis has one) and centers the crop on it, clamped so the
        crop stays fully inside the die's own bbox. Only called when at
        least one axis has more than one block (caller-guaranteed)."""
        p = self.p
    ...
    def _build_block(self, br: int, bc: int, tmp_dir: str) -> None:
        ...
        # If block_presets is set, this block's jitter base is a randomly
        # chosen preset instead of the die's own single base DRAMParams --
        # this draw must happen before _randomize_block_params so it doesn't
        # disturb that function's own rng draw sequence. block_presets=None
        # (falsy) skips this entirely: block_rng.integers is never called,
        # so the rng state feeding _randomize_block_params is untouched and
        # output for that case stays byte-identical to before this feature.
        block_base = p
        if p.block_presets:
    ...
    def generate(self) -> dict:
        ...
        # boundary_bias defaults to 0.0, so `p.boundary_bias > 0.0` is False
        # and short-circuits before self.rng.random() is ever called --
        # rng state (and therefore all downstream output) for the default
        # case is untouched by this branch existing.
...
def _supersampled_masks_for_bbox(gds_path: str, bbox_nm: list, ppn_ss: float,
    ...
    _layout: reuse an already-parsed db.Layout instead of re-reading
    gds_path from disk -- generate_sample() passes its own single parse
    through here for both the search and reference renders. Defaults to
    None (read gds_path fresh, the original behavior) so every other
    ...
    # each_touching() uses KLayout's internal spatial index to skip shapes
    # that can't possibly intersect this bbox, instead of Python-looping
    # over every shape in the layer and discarding out-of-range ones one at
    # a time -- a huge win for a small reference-patch crop out of a
    # much larger array (measured ~100x fewer candidate shapes on some
    # layers). The exact per-shape pixel-overlap check below is unchanged,
    # so this can only narrow the candidate set, never change the result --
    # PROVIDED the query box matches the per-shape math's own true window,
    # not bbox_nm itself: when ss_w/ss_h don't match bbox_nm's aspect ratio
    # (e.g. a non-square array against a square supersample canvas), the
    # clamps below (min(ss_w, ...) / min(ss_h, ...)) already let shapes
    # just outside bbox_nm bleed into that margin -- confirmed empirically
    # (byte-diff against the pre-optimization renderer) before this fix.
    # +/-1nm pad guards against a shape sitting exactly on the boundary
    # being missed due to floor/ceil.
    ...
    for ln, dt in layer_list:
        ...
        # Every caller immediately does `masks[layer] > 0.5` -- mask only
        # ever holds 0 or 255 (no antialiasing at this stage; that happens
        # later via intensity-buffer downsampling), so a bool view is
        # exactly equivalent and skips a full-array float32 copy + divide
        # per layer per render.
        masks[(ln, dt)] = mask > 0
    return masks
...
def render_sem_image(
    ...
    Basic pipeline: layer fill + edge enhancement only. No beam blur, shot/read
    noise, or gain/offset jitter -- those are added back in a later pass."""
    ...
    # Any margin where content doesn't fill the full canvas (array/die isn't
    # square) stays flat SEM_BG here -- honest "no content", not a repeated
    # lattice that could look like real structure.
...
def render_sem_patch(
    ...
    arbitrary pixel scale. Same basic pipeline as render_sem_image (layer
    fill + edge enhancement only, no noise), but for a caller-specified crop
    (used for the high-resolution reference patch) instead of the whole
    array. No margin tiling: the caller picks width_px/height_px to exactly
    match bbox_nm at ppn, so content always fills the canvas exactly.
    """
...
DEFAULT_PARAMS_KW = dict(
    rows=64, cols=85,   # square array (see DRAMParams.cols) -- fills the canvas, no margin
    cell_pitch_nm=80.0, cell_pitch_bl_nm=60.0,
    wl_width_nm=24.0, bl_width_nm=18.0,
    contact_size_nm=40.0, capacitor_size_nm=30.0,
...
def generate_sample(seed: int, tmp_dir: str, **params_kw) -> ReferenceSearchSample:
    ...
    # write_metadata_json=False: this metadata is used directly from the
    # returned dict below and json_path is unlinked at the end of this
    # function without ever being read -- writing it out first (it can hold
    # tens of thousands of cell/wordline/bitline coordinates) is pure waste.
    p = DRAMParams(**kw, seed=seed, output_gds=gds_path, output_json=json_path,
                   write_metadata_json=False)
    gen = DieGenerator(p)
    meta = gen.generate()

    # Parsed once and reused for both renders below -- the file is immutable
    # between them, so a second read()/parse of the same GDS was pure waste.
    layout = db.Layout()
    layout.read(gds_path)
    ...
    # -- Reference image: same physical patch, rendered at zoom_ratio x the
    # search image's pixel density (zoom_ratio=1 -> literally reference_width_px
    # x reference_height_px, i.e. 100x100) --
