...
def test_dram_generator_accepts_true_zoom_reference_size(tmp_path):
    p = m.DRAMParams(reference_width_px=100, reference_height_px=100, zoom_ratio=10,
                      output_gds=str(tmp_path / "x.gds"), output_json=str(tmp_path / "x.json"),
                      seed=1)
    m.DRAMGenerator(p)  # must not raise
...
def test_generate_sample_default_zoom_ratio_unchanged(tmp_path):
    # Regression guard: not passing zoom_ratio must still behave exactly as
    # before (100x100 reference at zoom_ratio=1).
...
def test_generate_sample_default_single_block_unchanged(tmp_path):
    ...
    assert sample.reference_img.shape[:2] == (100, 100)  # 100px * zoom_ratio(1)
