...
# Distribution family for the position-jitter draw below ("normal" or
# "uniform"). This is the knob ablation A1's "shifted" profile flips: it is
# the aperiodicity signal (position jitter is what keeps the lattice from
# being exactly periodic), so switching its family -- not just its magnitude
# -- tests whether a trained model generalises to aperiodicity in general or
# overfit to one specific noise distribution. Width jitter (above) always
# stays gaussian; it is a CD-variation proxy, not the ablation's subject.
POSITION_JITTER_DIST = "normal"


def _position_jitter(rng: np.random.Generator) -> float:
    """One position-jitter draw, honouring POSITION_JITTER_DIST.

    Uniform's half-width is scaled by sqrt(3) so both families share the
    same standard deviation (POSITION_JITTER_NM) -- "magnitude" means the
    same thing regardless of which family is selected, isolating family as
...
def generate_dram_canvas(
    ...
    # word_line_layer/bit_line_layer/contact_layer are only ever consumed via
    # the `layers` dict below, which only exists when return_layers=True --
    # the only caller in the actual data-generation hot path (mxn.py) never
    # passes it. Allocating three more full-size uint8 canvases and drawing
    # every contact circle a second time into contact_layer was pure waste
    # on that path; both are now conditional on actually needing them.
    ...
    # Checkerboard-selected (word, bit) positions, in the same i-outer/
    # j-inner order the original nested loop visited them in. One batched
    # rng.normal() draw for all their radius-jitter values instead of one
    # call per contact inside the loop -- NumPy's Generator consumes its
    # underlying bitstream in this same row-major sequence either way
    # (verified bit-identical against the original per-contact-call form,
    # same property already relied on for AFB's add_ler equivalent fix), so
    # this is a pure speedup, not a behavior change.
